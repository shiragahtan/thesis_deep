...
\begin{enumerate}
  \item \textbf{Primes and strings benchmarks}: the sieve and KMP are
    achievable in pure Python with dramatic speedups — a better showcase
    for smart-step reasoning.
  \item \textbf{3 seeds per config}: for statistical significance.
  \item \textbf{Mixed config} (70\% smart, 30\% dumb): the actual thesis proposal.
  \item \textbf{Smart-only on matrix task}: can the LLM rediscover Strassen?
\end{enumerate}
